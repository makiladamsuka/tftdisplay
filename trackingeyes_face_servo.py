...
NO_FACE_SLEEPY_SEC = 3.0
NO_FACE_BORED_SEC = 9.0
...
def servo_worker():
    global servo_current_pan, servo_current_tilt, servo_running
...
def vision_worker():
    ...
    global target_face_present, target_face_area_ratio
    ...
    while running:
        ...
        if local_face_present:
            ...
            if no_face_elapsed >= NO_FACE_BORED_SEC:
                ...
                target_emotion = "sleepy"
            else:
                target_emotion = "idle"

        # Debug: Show emotion state at each frame
        if DEBUG_EMOTIONS:
            state_info = f"[{target_emotion:8}] face_present={local_face_present} area_ratio={local_face_area_ratio:.3f} close={router_face_close}"
            if local_face_present:
        ...
        if target_emotion != current_emotion:
            ...
            if immediate_excited or (hold_ok and cooldown_ok):
                left_eye.set_emotion(target_emotion, 1.0)
